...
// ACF fields might not be attached to attribute terms by default unless configured.
// We keep the logic generic.
...
if (!$banner_desc) {
    $banner_desc = 'Nâng tầm bầu không khí không gian sống, nơi cảm xúc, sự hiện diện và tĩnh lặng hòa quyện.';
}

// Intro Section Fields
$intro_title = get_field('intro_title', $term) ?: 'NYN Atmosphere Collection';
$intro_subtitle = get_field('intro_subtitle', $term) ?: 'Travel as a state of mind. Defined by scent.';
$intro_c1 = get_field('intro_content_1', $term);
$intro_c2 = get_field('intro_content_2', $term);

// Highlights Section Fields
$highlights_title = get_field('highlights_title', $term) ?: 'The Collection';
$highlights_subtitle = get_field('highlights_subtitle', $term) ?: 'Ba mùi hương. Ba điểm đến. Ba trạng thái cảm xúc - được chuyển hóa thành hương.';
...
$info_title = get_field('info_title', $term) ?: 'About the Fragrances';
...
<main>
	...
	<section class="product-1 section-py">
		<div class="container">
			<div
				class="wrap-heading mb-base md:gap-0 md:flex-row flex-col items-start  flex md:items-center justify-between">
	...
	<!-- Content Sections (Intro, Highlights, Info) can be reused if ACF is enabled for attributes -->
	<!-- For simplicity, we keep the HTML structure but checks will likely fail if no data -->
		...
		<div class="container">
			<div class="wrap-heading rem:max-w-[920px] w-full mx-auto text-center font-extrabold">
				...
				<div class="sub-title rem:text-[30px] mb-8" data-aos="fade-up" data-aos-delay="100">
					<?php echo esc_html($intro_subtitle); ?></div>
				<?php endif; ?>
	...
	<section class="product-4 section-py"
		...
		<div class="container">
			<div
				...
				<div class="sub-title rem:text-[30px] mb-8" data-aos="fade-up" data-aos-delay="100">
					<?php echo esc_html($info_subtitle); ?></div>
				<div class="line rem:h-[2px] rem:w-[57px] bg-Utility-Black mx-auto" data-aos="zoom-in"
					data-aos-delay="200"> </div>
